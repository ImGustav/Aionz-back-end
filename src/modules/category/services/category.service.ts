...
export class CategoryService implements CategoryServiceInterface{
  ...
  async create(createCategoryDto: CreateCategoryDto): Promise<ResponseCategory> {
    const nameExist = await this.categoryRepository.findByName(createCategoryDto.name)
    if(nameExist){
      throw new ConflictException("Category with the same name already exist.")
    }

    const categoryToCreate = await this.categoryRepository.create(createCategoryDto)
  ...
  async update(id: number, updateCategoryDto: UpdateCategoryDto): Promise<ResponseCategory>{
    ...
    if(updateCategoryDto.name){
      const nameExist = await this.categoryRepository.findByName(updateCategoryDto.name)
      if(nameExist && nameExist.id !== id){
        throw new ConflictException("Category with the same name already exist.")
      }
    }
    
    const categoryToUpdate = await this.categoryRepository.update(id, updateCategoryDto)
